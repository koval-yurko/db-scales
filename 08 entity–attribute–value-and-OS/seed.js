// Configurable batch generation from the CLI (unchanged contract vs `07`).
// Products are written to PostgreSQL and each enqueues an outbox 'upsert', so a
// running `npm run sync` worker (or a later `npm run reindex`) pushes them into
// OpenSearch.
//
...
const { insertProductsBatch, pendingOutboxCount } = require('./utils/repository');
...
async function main() {
  ...
  console.log(`  ${await pendingOutboxCount()} outbox events pending — run \`npm run sync\` (or \`npm run reindex\`) to index them.`);
  await closePool();
}
