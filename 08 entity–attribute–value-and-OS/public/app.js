// Minimal vanilla-JS UI. Forms/filters are built from attribute metadata (from
// PostgreSQL); the results, facets and full-text all come from OpenSearch. The
// facet sidebar + search box are what the search engine adds over `07`.
...
async function init() {
  ...
  renderSeedAttrs();
  refreshSyncStatus();
  setInterval(refreshSyncStatus, 4000);
}
...
async function refreshSyncStatus() {
  try {
    ...
    $('sync-status').textContent =
      `indexed: ${s.indexedDocs.toLocaleString()} docs · outbox pending: ${s.pendingOutbox}`;
  } catch (e) { /* ignore */ }
}
...
// which attributes to request facets for: filterable text/bool + price
function facetCodes(cat) {
  const codes = cat.attributes
    .filter((a) => a.is_filterable && (a.data_type === 'text' || a.data_type === 'bool'))
    .map((a) => a.code);
  codes.push('price');
  return codes;
}
...
function buildFilterQuery(page) {
  ...
  params.set('facets', facetCodes(cat).join(','));
  params.set('page', page);
  if ($('show-query').checked) params.set('explain', '1');
  return params;
}
...
// click a facet value -> set the matching filter control and re-run
function applyFacet(code, value) {
  if (code === 'price') return;
  const el = $('filter-fields').querySelector(`[data-code="${code}"][data-kind="text"], [data-code="${code}"][data-kind="bool"]`);
  if (el) { el.value = value; applyFilter(1); }
}

function renderFacets(facets) {
  const host = $('facets');
  if (!facets) { host.innerHTML = ''; return; }
  const cat = byCode($('filter-category').value);
  const labelOf = (code) => code === 'price' ? 'Price' : (cat.attributes.find((a) => a.code === code)?.label || code);

  host.innerHTML = Object.entries(facets).map(([code, f]) => {
    if (Array.isArray(f)) {
      const rows = f.slice(0, 8).map((b) =>
        `<div class="bucket" data-code="${code}" data-value="${b.value}"><span>${b.value}</span><span class="c">${b.count}</span></div>`).join('');
      return `<div class="facet"><h3>${labelOf(code)}</h3>${rows || '<div class="meta">—</div>'}</div>`;
    }
    return `<div class="facet"><h3>${labelOf(code)}</h3>
              <div class="meta">min ${f.min ?? '—'}<br>avg ${f.avg != null ? f.avg.toFixed(2) : '—'}<br>max ${f.max ?? '—'}</div></div>`;
  }).join('');

  for (const b of host.querySelectorAll('.bucket')) {
    b.addEventListener('click', () => applyFacet(b.dataset.code, b.dataset.value));
  }
}

function renderResults(data) {
  $('results-meta').innerHTML =
    `<b>${data.total.toLocaleString()}</b> match · page ${data.page} · OpenSearch ${data.ms}ms (took ${data.took}ms)` +
    (data.filters?.length ? ` · ${data.filters.map((f) => `${f.field} ${f.op} ${f.value}`).join(', ')}` : '');
  ...
  if (data.query) {
    ...
    box.textContent = 'POST /products/_search\n' + JSON.stringify(data.query, null, 2);
  } else {
...
async function saveProduct() {
  ...
  try {
    const saved = id
      ...
      : await api('/api/products' + qs, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    $('form-status').textContent = `Saved product #${saved.id}` + (saved.indexed ? ' · indexed ✓' : '');
    if (!id) resetForm();
    refreshSyncStatus();
  } catch (e) {
...
async function runSeed() {
  ...
  try {
    const r = await api('/api/seed?wait=1', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    $('seed-status').textContent = `Inserted ${r.inserted} ${r.category} products in ${r.ms}ms` + (r.indexed ? ' · indexed ✓' : ' · pending sync');
    refreshSyncStatus();
  } catch (e) {
